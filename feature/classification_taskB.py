...
class Classification:
    def __init__(self, classifier, trainFile, testFile):
        ...
        X_train = data[:, 1:-1]         # cid/qid features label
        y_train = data[:, -1]
        # y_train = getOneHotVec(y_train, GENERAL_NUM_CLASS)
            
        # f = open(devFile)
        # pre = np.loadtxt(f)
        X_test = test_data[:, 1:]       # cid/qid features
        # y_test = test_data[:, -1]
        self.id = test_data[:, 0]
    ...
    def getPreResult(self, outputFile, type):
        ...
        for idx, each in enumerate(self.y_pred):
            # eachint = int(each)     # float
            fout.write(self.id[idx] + "\t" + self.int2Label(int(each), type) + "\n")
        fout.close()        
...
# taskB
# python classification_taskB.py essemble ./train_total_taskB.pkl ./devel_total_taskB.pkl  ./devel_subtaskB_result.txt 1
